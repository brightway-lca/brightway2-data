...
class ActivityProxyBase(ProxyBase):
    ...
    def valid(self, why=False):
        from .backends.base import sqlite_database_exists

        errors = []
        if not self.get("database"):
            errors.append("Missing field ``database``")
        elif not sqlite_database_exists(self.get("database")):
...
class ExchangeProxyBase(ProxyBase):
    ...
    def valid(self, why=False):
        ...
        elif not sqlite_database_exists(self["input"][0]):
            errors.append(
        ...
        elif not sqlite_database_exists(self["output"][0]):
            errors.append(
