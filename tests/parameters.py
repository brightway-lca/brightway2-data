...
from bw2data.parameters import (ActivityParameter, DatabaseParameter, Group,
                                GroupDependency, ParameterizedExchange,
                                ProjectParameter, parameters)
...
def test_project_parameters():
    ...
    obj = ProjectParameter.create(
        name="foo", amount=3.14, data={"uncertainty type": 0}
    )
    assert obj.name == "foo"
...
def test_project_parameter_dependency_chain():
    ...
    expected = [
        {"kind": "project", "group": "project", "names": {"foo"}},
    ]
...
def test_project_parameter_dependency_chain_missing():
    ...
    ProjectParameter.create(name="baz", amount=8, formula="foo * bar_")
...
def test_project_parameter_is_not_deletable_database():
    ...
    DatabaseParameter.create(
        database="B", name="bar", amount=1, formula="foo * 5"
    )
...
def test_database_parameters_static():
    ...
    DatabaseParameter.create(
        database="bar", name="baz", amount=7, formula="foo + baz"
    )
...
def test_database_parameter_dependency_chain():
    ...
    expected = [
        {"kind": "project", "group": "project", "names": {"fly", "hike"}},
    ]
...
def test_database_parameter_dependency_chain_include_self():
    ...
    expected = [
        {"kind": "project", "group": "project", "names": {"fly"}},
        {"kind": "database", "group": "B", "names": {"car"}},
    ]
    # Method now also includes required names within group
    assert (
        DatabaseParameter.dependency_chain("B", include_self=True) == expected
    )
...
def test_database_parameter_is_not_deletable_database():
    ...
    DatabaseParameter.create(database="B", name="car", amount=8)
    DatabaseParameter.create(
        database="B", name="truck", formula="car * 5", amount=4
    )
...
def test_database_parameter_is_dependent_on():
    ...
    DatabaseParameter.create(
        database="B", name="baz", amount=1, formula="foo + 2"
    )
    parameters.recalculate()
...
def test_database_parameter_formula_update_project():
    ...
    Database("C").register()
    DatabaseParameter.create(
        database="B", name="bar", amount=1, formula="foo + 2"
    )
    DatabaseParameter.create(
        database="C", name="bing", amount=1, formula="foo + 2"
    )
    DatabaseParameter.create(
        database="C", name="foo", amount=8, formula="tracks * 2"
    )
    parameters.recalculate()
...
def test_create_parameterized_exchange_missing_group():
    with pytest.raises(IntegrityError):
        ParameterizedExchange.create(
            group="A", exchange=42, formula="foo + bar"
        )
...
def test_activity_parameter_static(chain):
    ...
    assert (
        ActivityParameter.static("A", full=True, only=["foo", "bar"])
        == expected
    )
...
def test_activity_parameter_dependency_chain(chain):
    expected = [{"kind": "activity", "group": "A", "names": {"D", "F"}}]
    assert ActivityParameter.dependency_chain("G") == expected
    expected = [
        {"kind": "database", "group": "B", "names": {"foo"}},
        {"kind": "project", "group": "project", "names": {"bar"}},
    ]
...
def test_activity_parameter_dependency_chain_includes_exchanges(chain):
    ...
    db = Database("K")
    a = db.new_activity(
        code="something something danger zone", name="An activity"
    )
    a.save()
    a.new_exchange(
...
def test_activity_parameter_dependency_chain_include_self(chain):
    ...
    assert ActivityParameter.dependency_chain("A") == expected
    expected = [
        {"kind": "activity", "group": "A", "names": {"D"}},
        {"kind": "database", "group": "B", "names": {"foo"}},
        {"kind": "project", "group": "project", "names": {"bar"}},
    ]
    assert (
        ActivityParameter.dependency_chain("A", include_self=True) == expected
    )
...
def test_activity_parameter_dependency_chain_include_self_exchanges(chain):
    ...
    a.save()
    a.new_exchange(
        amount=0, input=a, type="production", formula="15 / H"
    ).save()
    parameters.add_exchanges_to_group("G", a)
    ...
    expected = [
        {"kind": "activity", "group": "A", "names": {"D", "F"}},
        {"kind": "activity", "group": "G", "names": {"H"}},
    ]
    assert (
        ActivityParameter.dependency_chain("G", include_self=True) == expected
    )
...
def test_activity_parameter_depend_within_group_include(chain):
    ...
    assert ActivityParameter.is_dependency_within_group(
        "F", "G", include_order=True
    )
...
def test_recalculate_exchanges_no_activities_parameters():
    ...
    a.save()
    a.new_exchange(
        amount=0, input=a, type="production", formula="foo + 4"
    ).save()
...
def test_activity_parameter_formula_update_project(chain):
    ...
    parameters.recalculate()
    assert (
        ActivityParameter.get(name="F", group="A").formula == "foo + bar + D"
    )
    ...
    assert (
        ActivityParameter.get(name="F", group="A").formula
        == "foo + banana + D"
    )
    assert ActivityParameter.get(name="baz", group="G").formula == "bar * 6"
...
def test_activity_parameter_formula_update_database(chain):
    ...
    assert (
        ActivityParameter.get(name="F", group="A").formula == "mango + bar + D"
    )
    assert ActivityParameter.get(name="baz", group="G").formula == "foo * 6"
...
def test_activity_parameter_formula_update_activity_include(chain):
    ...
    assert (
        ActivityParameter.get(name="J", group="G").formula == "F + dingo * 2"
    )
...
def test_activity_parameter_checks():
    with pytest.raises(IntegrityError):
        ActivityParameter.create(
            group="project", database="E", name="F", code="G"
        )
    with pytest.raises(IntegrityError):
...
def test_parameters_rename_project_parameter():
    ...
    assert (
        ProjectParameter.select().where(ProjectParameter.name == "foo").count()
        == 1
    )
    ...
    assert (
        ProjectParameter.select()
        .where(ProjectParameter.name == "foobar")
        .count()
        == 1
    )
...
def test_parameters_rename_project_parameter_dependencies_full(chain):
    ...
    DatabaseParameter.create(
        database="B", name="bing", amount=2, formula="bar ** 5"
    )
    parameters.recalculate()
    ...
    parameters.rename_project_parameter(
        param, "new_bar", update_dependencies=True
    )

    assert ProjectParameter.get(name="double_bar").formula == "new_bar * 2"
    assert (
        DatabaseParameter.get(name="bing", database="B").formula
        == "new_bar ** 5"
    )
    assert (
        ActivityParameter.get(name="F", group="A").formula
...
def test_parameters_rename_database_parameter():
    ...
    assert (
        ...
        .where(DatabaseParameter.name == "foo")
        .count()
        == 1
    )
    ...
    assert (
        DatabaseParameter.select()
        .where(DatabaseParameter.name == "bar")
        .count()
        == 1
    )
...
def test_parameters_rename_activity_parameter(chain):
    ...
    assert (
        ...
        .where(
            ActivityParameter.name == "John", ActivityParameter.group == "G"
        )
        .count()
        == 1
    )
...
def test_parameters_rename_activity_parameter_group_exchange():
    ...
    a.save()
    a.new_exchange(
        amount=1, input=a, type="production", formula="D + 2"
    ).save()
...
def test_parameters_rename_activity_parameter_order_exchange():
    ...
    parameters.rename_activity_parameter(
        param, "Correct", update_dependencies=True
    )
...
def test_parameters_remove_from_group():
    ...
    b.save()
    a.new_exchange(
        amount=0, input=b, type="technosphere", formula="bar + 4"
    ).save()
    activity_data = [
        {
            "name": "reference_me",
...
def test_parameters_save_restore_exchange_amount():
    ...
    b.save()
    a.new_exchange(
        amount=5, input=b, type="technosphere", formula="bing + 5"
    ).save()
